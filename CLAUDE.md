...
cargo test --workspace                   # run all tests (66 tests)
...
**Workspace layout:** 3 crates with a clear dependency chain.
...
- **generate.rs** — `RandomGen` (probabilistic) and `TemplateGen` (predefined patterns) both implement `Generator` trait
...
- **client.rs** — `LlmClient` trait + `ClaudeClient` (with exponential backoff retry) + `MockClient` for tests
