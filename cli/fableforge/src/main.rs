...
use fableforge_core::{
    Connective, Formula, GenConfig, Generator, Lang, RandomGen,
};
use fableforge_llm::{
    AnyClient, ClaudeClient, CoherenceEvaluator, CoherenceReport,
    OpenRouterClient, StoryComposer, StyleConfig,
};
...
fn parse_lang(s: &str) -> Result<Lang> {
    match s.to_lowercase().as_str() {
        ...
        "ru" => Ok(Lang::Ru),
        other => bail!(
            "Unknown language '{}'. Supported: en, ru",
            other
        ),
    }
}
...
async fn generate_tale(args: GenerateArgs) -> Result<()> {
    ...
    if evaluate {
        ...
        let eval_client = build_client(
            eval_api_key.unwrap(),
            eval_model.unwrap(),
        );
        let evaluator =
            CoherenceEvaluator::new(eval_client, eval_style.unwrap())
                .with_lang(lang);
...
fn show_structure(
    ...
    if export_formula {
        println!(
            "{}",
            Formula::from_tale(&tale).to_text()
        );
    } else if latex {
        println!(
            "{}",
            Formula::from_tale(&tale).to_latex()
        );
    } else {
        print_structure(&tale, lang);
    }
    ...
    match lang {
        Lang::En => {
            ...
            println!(
                ...
                d.character_consistency
            );
            println!(
                "  Structural fidelity:   {:.2}",
                d.structural_fidelity
            );
            println!(
                "  Episode continuity:    {:.2}",
                d.episode_continuity
            );
            println!(
                "  Narrative quality:     {:.2}",
                d.narrative_quality
        ...
        Lang::Ru => {
            ...
            println!("Критерии:");
            println!(
                "  Соответствие персонажей:    {:.2}",
                d.character_consistency
            );
            println!(
                "  Верность структуре:         {:.2}",
                d.structural_fidelity
            );
            println!(
                "  Связность эпизодов:         {:.2}",
                d.episode_continuity
            );
            println!(
                "  Качество повествования:     {:.2}",
                d.narrative_quality
            );
