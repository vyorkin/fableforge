...
//!
//! A fairy tale consists of an optional initial situation followed by one or
//! more moves. Each move is a sequence of moments (functions with
//! participants).
//!
...
impl Tale {
    ...
    pub fn all_moments(&self) -> impl Iterator<Item = &Moment> {
        self.moves.iter().flat_map(|m| {
            m.moments
                .iter()
                .chain(m.embedded_moves.iter().flat_map(|em| em.moments.iter()))
        })
    }
}
...
///
/// Per Propp, this typically includes family enumeration and state of
/// prosperity, but both are optional. Some tales start directly with a
/// function.
